document.addEventListener('DOMContentLoaded', function () {
  ...
  let myEvents = []; // Khởi tạo mảng sự kiện

  // Gọi API từ Servlet để lấy dữ liệu lịch
  fetch('http://localhost:9999/ChildCareProject/DoctorSchedule')
    .then(response => response.json())
    .then(data => {
      myEvents = data; // Cập nhật dữ liệu lịch từ Servlet
      initializeCalendar(); // Khởi tạo lịch sau khi nhận được dữ liệu
    })
  ...
  function initializeCalendar() {
    const calendar = new FullCalendar.Calendar(calendarEl, {
      // Cấu hình lịch ở đây
      events: myEvents, // Sử dụng dữ liệu lịch từ Servlet
      // ...
    });
    
    calendar.render(); // Render lịch
  }
